...
const avatarUrls = indianNames.map((name) => `https://api.dicebear.com/7.x/thumbs/svg?seed=${name.replace(' ', '')}`);
async function seed() {
    await mongoose_1.default.connect(MONGO_URI);
    console.log('✅ Connected to MongoDB');
    const hashedPassword = await bcrypt_1.default.hash(DUMMY_PASSWORD, SALT_ROUNDS);
